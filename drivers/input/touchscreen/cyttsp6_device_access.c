...
#define OPENS_TEST_TYPE_MUTUAL	0
#define OPENS_TEST_TYPE_BUTTON	1

#define STATUS_SUCCESS	0
#define STATUS_FAIL	-1
#define I2C_BUS_SIZE   (8+246)  /*Header + data size*/
#define HEADER_BYTES   8	/*Header size*/

#define MAX_SYSFS_BUF_LEN	650

#define GET_CAP_OFFSET 		0
#define DATA_INFO 		5
#define DATA_CRC 		2
#define HARDWARE_DATA_LENGTH 	246

static u16 data_read_length;		/*given by hardware i.e., (HARDWARE_DATA_LENGTH + 5) 
						  1st 5 bytes is info*/

static u16 input_len; 			/*Given by user*/
static u16 loop_len = 1 ;

static u16 null_cmd_current_size;
		...
		size_t length)
{
	...
	if (dad->test.cur_cmd == CY_CMD_CAT_NULL) {
		...
		//num_read = dad->test.cur_status_size;
		if(ic_buf[0] == 0x08) 
			num_read = null_cmd_current_size; 
		else
			num_read = dad->test.cur_status_size;
		dev_vdbg(dev, "%s: Updated num_read  = %d \n",
				__func__, num_read);
		if((num_read) > I2C_BUS_SIZE) {

			dev_err(dev, "%s : Warning requested data size is greater than I2C buffer size",
					__func__);
			num_read = I2C_BUS_SIZE;
		}
		if (length < num_read) {
			dev_err(dev, "%s: not sufficient buffer %s=%d, %s=%zu\n",
					__func__, "req_buf_len", num_read,
		...
		struct device_attribute *attr, char *buf)
{
	...
	for (i = 0; i < num_read; i++) {
			if (i == 5) {
				...
				num_read_size = num_read - HEADER_BYTES;
				index += scnprintf(&buf[index], CY_MAX_PRBUF_SIZE - index,
						"0x%02X\n", num_read_size);
			} else {
				/*
				 * Every time it occupies 5 Bytes in buf
				 */
				index += scnprintf(&buf[index], CY_MAX_PRBUF_SIZE - index,
						"0x%02X\n", dad->ic_buf[i]);
			}
		...
		size_t length)
{
	...
	if (dad->test.cur_cmd == CY_CMD_CAT_NULL) {
		...
		cyttsp6_pr_buf(dev, NULL, ic_buf, length, "test_cmd");
		/*
		 * 0x08 is get self data to be run after 07
		 */
		if((dad->test.cur_mode == CY_MODE_CAT) && 
			(ic_buf[0] == 0x08))	{
			/*
			 * 8 is reponse header.
			 */
			null_cmd_current_size = ((ic_buf[3] << 8) +
									ic_buf[4] + 8); 
		}
 ...
 */
static int _cyttsp6_run_opens_self_test_cmd(struct device *dev,
...
static int _cyttsp6_get_cap_self_test_results_cmd(struct device *dev,
		u8 test_id, u16 offset, u16 length, u8 *status,
		u16 *act_length, u8 *data)
{
	...
	if (rc)
		goto exit;
	data_read_length = ((ret_buf[2] << 8) + ret_buf[3] + DATA_INFO);

	if (data_read_length && data) {
		/* Read test result data */
		rc = cmd->read(dev, CY_MODE_CAT, CY_REG_CAT_CMD + 1,
				data, data_read_length);
		if (rc)
			goto exit;
	}
...
release_exclusive:
	...
	num_bytes = ((dad->ic_buf[7]) & 0x03);
	num_bytes *= dad->heatmap.num_element ;
	dev_vdbg(dev, "%s: Total number of bytes = %d \n",
			__func__, num_bytes);
...
cyttsp6_get_panel_data_store_exit:
	...
	if ((dad->heatmap.num_element + 20) > (CY_MAX_PRBUF_SIZE / 2)) {
		dev_err(dev, "ERROR:Scan Data length: (%d) exceeds the limit: (%d) bytes.\n",
				dad->heatmap.num_element, (CY_MAX_PRBUF_SIZE / 2));
	}
...
/*
 * TO print the out put of Cp and Cm capacitance
 */
static int get_cap_prepare_print_buffer(int status, u8 *in_buf, int length,
		u8 *out_buf, u8 cap_test_type)
{
	static int i_index, c_cnt, total_bytes;
	int i = 0, j = 0;
	u8 input_len_msb = 0, input_len_lsb = 0;
	if (i_index == 0) {
		i_index += scnprintf(out_buf, CY_MAX_PRBUF_SIZE,
				"status %02X\n", status);
	} else {
		c_cnt ++;
		j = DATA_INFO;
	...
	/*
	 * To copy the data to buffer
	 */
	for (i = j; i < length; i++) {
		total_bytes++;
		if ((c_cnt == 0) && ((i == 2) || (i == 3))) {
			input_len_msb = (((input_len)
					& 0xffff)>>8);
			input_len_lsb = ((input_len)
					& 0xff);
			/*input_len_msb = (((input_len - DATA_INFO - DATA_CRC)
					& 0xffff)>>8);
			input_len_lsb = ((input_len - DATA_INFO - DATA_CRC)
					& 0xff);
		*/	if (i == 2) {
				i_index += scnprintf(&out_buf[i_index],
						CY_MAX_PRBUF_SIZE - i_index,
						"%02X", input_len_msb);
			} else {
				i_index += scnprintf(&out_buf[i_index],
						CY_MAX_PRBUF_SIZE - i_index,
	...
	j = i_index;
	/*
	 * To check the overflow of index (Because index and c_cnt is static)
	 */
	if ((c_cnt) == (loop_len - 1)) {
		i_index += scnprintf(&out_buf[i_index],
				CY_MAX_PRBUF_SIZE - i_index,
...
static int _cyttsp6_get_panel_test_results(struct device *dev,
		u8 test_type, u8 *status, u16 *act_length,
		u8 *data, u8 grp_cap_id)
{
	static u16 pt_offset, pt_length,loop_count ;
	u16 res=0,i=0;

	u8 test_id = CY_ST_ID_CM_PANEL_TEST + test_type;

	if((grp_cap_id == 1)&&(CY_ST_ID_CM_PANEL_TEST == 5)) {
		loop_count++;
		i = 1;
		if(loop_count == 1) {
			pt_offset = GET_CAP_OFFSET ;
			pt_length = input_len;

		}

		res = _cyttsp6_get_cap_self_test_results_cmd(dev, test_id,
				pt_offset, pt_length, status, act_length, data);

		pt_offset += (data_read_length - DATA_INFO );
		pt_length -= ( data_read_length - DATA_INFO ); /* 1ST 5BYTES WILL BE INFO */
...
static ssize_t cyttsp6_get_panel_capacitance_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct cyttsp6_device_access_data *dad
		= cyttsp6_get_device_access_data(dev);
	...
	int length = 0;
	int size = 0;
	int rc, l_cnt = 0,sum = 0;

	for( loop_len = 0 ; sum < input_len ; loop_len++)
		sum += (HARDWARE_DATA_LENGTH);

	mutex_lock(&dad->sysfs_lock);
	pm_runtime_get_sync(dev);

	while (l_cnt < loop_len) {
		...
		if (l_cnt == 0) {
			...
			if (dad->cap_target == CAP_TARGET_PANEL)
				rc = _cyttsp6_run_panel_test(dev,
					dad->cap_test_type, &cmd_status,
					&summary_result);
			else if (dad->cap_target == CAP_TARGET_BUTTON)
				rc = _cyttsp6_run_button_test(dev,
					dad->cap_test_type,
			...
			if (rc < 0) {
				dev_err(dev,
					"%s: Error on run capacitance self test r=%d\n",
						__func__, rc);
		 ...
		 * The last parameter is used to diffrenciate between
		 * mfg-capacitance(i.e,. 0) and get_panel_capacitance(i.e,. 1)
		 */
		if (dad->cap_target == CAP_TARGET_PANEL)
			rc = _cyttsp6_get_panel_test_results(dev,
				dad->cap_test_type, &cmd_status, &act_length,
				dad->ic_buf, 1);
		else if (dad->cap_target == CAP_TARGET_BUTTON)
			rc = _cyttsp6_get_button_test_results(dev,
				dad->cap_test_type,
		...
		length = data_read_length;

		size += get_cap_prepare_print_buffer(status, dad->ic_buf,
				length, buf, dad->cap_test_type);
...
static ssize_t cyttsp6_get_panel_capacitance_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t size)
{
	struct cyttsp6_device_access_data *dad
		= cyttsp6_get_device_access_data(dev);
	...
	/*if ((dad->ic_buf[0] == 0x00) && (dad->ic_buf[1] == 0x00))
		input_len = ((dad->ic_buf[2] << 8) + (dad->ic_buf[3]))
			+ DATA_INFO + DATA_CRC;
	else
	*/	input_len = ((dad->ic_buf[2] << 8) + (dad->ic_buf[3]));
...
static DEVICE_ATTR(get_panel_capacitance, S_IRUSR | S_IWUSR,
	cyttsp6_get_panel_capacitance_show,
	cyttsp6_get_panel_capacitance_store);
...
status_success:
	if (dad->ic_buf[0] == CY_CMD_STATUS_SUCCESS &&
	    dad->ic_buf[1] == CY_ST_RESULT_PASS)
	status = STATUS_SUCCESS;
		...
		struct cyttsp6_attribute *attr, char *buf)
{
	...
	if (dad->cap_target == CAP_TARGET_PANEL)
		rc = _cyttsp6_get_panel_test_results(dev, dad->cap_test_type,
				&cmd_status, &act_length, dad->ic_buf, 0);
	else if (dad->cap_target == CAP_TARGET_BUTTON)
...
static int _cyttsp6_run_opens_self_test(struct device *dev,
		u8 *status, u8 *summary_result)
{
	int rc;

	rc = _cyttsp6_run_opens_self_test_cmd(dev, 1, status, summary_result,
			NULL);
	return rc;
}

static int _cyttsp6_get_opens_self_test_results(struct device *dev,
		u8 test_type, u8 *status, u16 *act_length, u8 *data)
{
	struct cyttsp6_device_access_data *dad
		= cyttsp6_get_device_access_data(dev);
	...
	rc = _cyttsp6_retrieve_data_structure_cmd(dev, 0, read_length, data_id,
	...
	rc = _cyttsp6_calibrate_idacs_cmd(dev, CY_CI_SM_MUTCAP_FINE, NULL);
	if (rc < 0) {
	...
	rc = _cyttsp6_calibrate_idacs_cmd(dev, CY_CI_SM_MUTCAP_BUTTON, NULL);
	if (rc < 0) {
...
static ssize_t cyttsp6_opens_show(struct device *dev,
		struct cyttsp6_attribute *attr, char *buf)
{
	struct cyttsp6_device_access_data *dad
		= cyttsp6_get_device_access_data(dev);
	...
	/* Run Opens Self Test */
	rc = _cyttsp6_run_opens_self_test(dev, &cmd_status,
			&summary_result);
	...
	rc = _cyttsp6_get_opens_self_test_results(dev,
...
static ssize_t cyttsp6_opens_store(struct device *dev,
		struct cyttsp6_attribute *attr, const char *buf, size_t size)
{
	struct cyttsp6_device_access_data *dad
		= cyttsp6_get_device_access_data(dev);
	...
	dad->opens_test_type = dad->ic_buf[0];
...
static CY_ATTR(opens, S_IRUSR | S_IWUSR,
	cyttsp6_opens_show, cyttsp6_opens_store);
...
static int cyttsp6_setup_sysfs(struct device *dev)
{
	...
	rc = device_create_file(dev, &dev_attr_get_panel_capacitance);
	if (rc) {
		dev_err(dev, "%s: Error, could not create get_panel_capacitance\n",
				__func__);
	...
	if (rc) {
		...
		goto unregister_get_panel_capacitance;
	}
	...
	rc = cyttsp6_create_file(dev, &cy_attr_opens);
	if (rc) {
...
unregister_opens:
	cyttsp6_remove_file(dev, &cy_attr_opens);
...
unregister_get_panel_capacitance:
	device_remove_file(dev, &dev_attr_get_panel_capacitance);  
